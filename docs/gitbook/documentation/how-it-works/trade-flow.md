...
| **3. Submit** | Client → Enclave | The signed order is sent over HTTPS to the attested gateway, which forwards it to the enclave over a mutually attested tunnel ([Transport & Attestation](/api-reference/getting-started/transport-and-attestation)). The enclave routes its signed symbol to one market, then verifies the signature, boot session, strictly increasing per-key nonce, viewing point, note opening, recent proof root, and input proof. It never touches a Solana transaction. |
