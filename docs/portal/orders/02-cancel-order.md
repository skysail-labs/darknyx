...
When an order is cancelled, the engine releases its collateral note and drops
the in-enclave continuation opening. A `cancelled` event is also emitted on the
