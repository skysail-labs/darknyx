...
if (!info) {
  console.log(`vault_config ${vaultConfig.toBase58()} already absent — nothing to close.`);
  process.exit(0);
}

const disc = createHash("sha256").update("global:close_vault_config").digest().subarray(0, 8);
...
console.log(`  vault_config closed: ${vaultConfig.toBase58()} (was ${info.data.length} bytes)`);
