...
//   * VaultConfig: protocol_owner_commitment + fee_rate_bps
...
const protocolDisc = createHash("sha256")
  ...
  .digest()
  .subarray(0, 8);
const protocolData = Buffer.alloc(8 + 32 + 2);
let o = 0;
...
console.log(
  `  fee_rate_bps=${FEE} enabled=${ENABLED} price_scale=${PRICE_SCALE} tick_size=${TICK} min_order_size=${MIN} circuit_breaker_bps=${CB}`,
);
console.log(
