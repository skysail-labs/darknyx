...
if (!RPC) throw new Error("SOLANA_RPC_URL is required (use the private Helius endpoint)");
if (!/^[0-9a-f]{64}$/.test(FEED)) throw new Error("feed id must be 32-byte hex");

const feedBytes = Buffer.from(FEED, "hex");
const shard = Buffer.alloc(2); // upgraded sponsored feeds use shard 0
const [account] = PublicKey.findProgramAddressSync([shard, feedBytes], PUSH);
const response = await new Connection(RPC, "finalized").getAccountInfoAndContext(account, {
  commitment: "finalized",
});
if (!response.value) throw new Error(`Pyth push account ${account} is missing`);
if (!response.value.owner.equals(RECEIVER)) throw new Error("unexpected Pyth account owner");

const data = response.value.data;
const messageOffset = 8 + 32 + 1;
const postedSlotOffset = messageOffset + 84;
if (data.length < postedSlotOffset + 8 || !data.subarray(0, 8).equals(DISCRIMINATOR)) {
...
if (!data.subarray(8, 40).equals(account.toBuffer())) throw new Error("wrong write authority");
