...
fn opening(mint: &Pubkey, amount: u64) -> DepositOpening {
    ...
    let owner_commitment = poseidon_hash(&[Fr::from(32u64), spending_key]).expect("owner hash");
    let inner_hash =
        poseidon_hash(&[Fr::from(33u64), recovery_nonce, note_secret]).expect("deposit inner hash");
...
fn valid_deposit_meets_size_and_cu_gates_and_invalid_proof_is_atomic() {
    ...
    // Poseidon3 under the fresh v2 domain. The owner is already bound by the
    // note commitment and must not be repeated in the inner derivation.
    assert_eq!(
        opening.inner_hash,
        poseidon_hash(&[Fr::from(33u64), opening.recovery_nonce, opening.note_secret,]).unwrap(),
    );
}
