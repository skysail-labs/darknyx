 ...
 * unit test — if the on-chain struct grows a field BEFORE these, move both.
 */
...
export function vaultConfigTeePubkeys(data: Uint8Array): string[] {
  if (data.length < NUM_TEE_KEYS_OFFSET + 1) {
    throw new Error("vault_config account data too short");
  }
