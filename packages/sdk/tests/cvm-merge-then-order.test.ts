...
import { be32ToBigInt, loadKeypairRel, StepTimer } from "./helpers/e2e-helpers.js";
import {
  CvmHarness,
  makePersona,
  gwFetch,
...
maybeDescribe("CVM merge-then-order (deposit×2 → merge → order off merged note)", () => {
  let cfg: E2EConfig;
  let conn: Connection;
  let admin: Keypair;
  let funder: Keypair;
  ...
  it(
    ...
    async () => {
      ...
      const Q = BigInt(process.env.NYX_CVM_BASE_QTY ?? String((Date.now() % 200_000) + 2000));
      ...
      await t.step("mint collateral", () =>
        sendAndConfirmTransaction(
          ...
          new Transaction().add(
            ...
            createMintToInstruction(baseMint, sellerBaseAta, admin.publicKey, A0 + A1),
      ...
      const nf0 = await nullifierV2(seller.spendingKey, sellerNote0.innerHash);
      const nf1 = await nullifierV2(seller.spendingKey, sellerNote1.innerHash);
      ...
      const mergedLeaf = Number(await readNoteMergedLeafIndex(conn, mergeSig));
      ...
      const sellerVI = await t.step("VALID_INPUT prove seller (merged note)", () =>
      ...
      async function submit(body: object): Promise<number> {
        ...
        if (!String(r.status).startsWith("2"))
          console.log(`  !! /orders ${r.status}: ${(await r.text()).slice(0, 200)}`);
        return r.status;
      }
      ...
      expect(String(sAsk).startsWith("2"), `ask rejected (${sAsk})`).toBe(true);
      expect(String(sBid).startsWith("2"), `bid rejected (${sBid})`).toBe(true);
