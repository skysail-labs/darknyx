...
Secrets/config (Helius key, CVM gateway, RUN_* flags) load from a gitignored
...
| `cvm-api-surface.test.ts` | error envelope + X-Request-Id, /system/status, /time, rate-limit 429, min_notional, idempotency, /account(+settings), WS seq + /ws/trading send-client + cancel-on-disconnect |
