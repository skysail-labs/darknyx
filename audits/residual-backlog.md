...
> **Update 2026-08-02:** `SW-07` (with `SW-24`) is **code complete** on
> `remediation/merkle-event-provenance` and owes only a CVM spot-check; the
> Helius key named in `SW-01` **has been rotated**, leaving that finding's code
> half (the endpoint that leaks it into `failed_reason`) open. `CA-01` is
> untouched and is now the highest-priority open item — it is also the third
> member of class C1 below, so the shared scope-and-provenance property this
> slice established applies to it directly.
