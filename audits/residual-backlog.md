...
> **Update 2026-08-02:** all three are addressed. `SW-07` (with `SW-24`) is
> **code complete** on `remediation/merkle-event-provenance`. `CA-01` (with
> `CA-02`/`CA-03`) is **code complete** on
> `remediation/attestation-measurement-binding` and needed no CVM at all. The
> Helius key named in `SW-01` **has been rotated**, leaving only that finding's
> code half — the endpoint that leaks it into `failed_reason`.
>
> With both Criticals closed, **class C1 (unscoped decoders — provenance, not
> parsing) is fully remediated**: all four decoders now establish the origin of
> what they read. Next up is the `SW-01` code fix, then `SW-02`/`SW-03`;
> `D-04`'s re-entry condition is one finding from being met.
