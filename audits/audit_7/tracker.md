...
|---|---|---|---|---|---|
| PF-12 | Perf | TEE journal | Slice 1 | Each batch transition performs one durable snapshot, not one per match; on-disk v2 format and write-ahead ordering stay unchanged. Unit test counts flushes; recovery drill records write p50/p95 after the reduction. | **Code complete** — N=16 unit fixture records one durable write; live recovery/settle evidence owed. |
| PF-13 | Perf-Nit | TEE settlement | Slice 1 | ALT activation waits before first poll and uses bounded backoff, reducing normal/degraded RPC calls without weakening the activation check. | **Code complete** — first guaranteed-useless poll removed; 12 s ceiling retained. |
| PF-14 | Perf-Nit | TEE API | Slice 1 | A transparency cache miss performs independent reserve reads concurrently while preserving owner/discriminator checks and stable response ordering. | **Code complete** — concurrent futures preserve mint order; full HTTP suite passed. |
| PF-15 | Perf-Nit | TEE prover | Slice 1 | `spawn_blocking` shares immutable N=16 witnesses rather than deep-cloning them. | **Code complete** — immutable `Arc<[MatchSlotWitness]>` crosses the blocking boundary. |
| PF-16 | Perf-Nit | TEE scheduler | Slice 1 | Final outcomes for one batch are applied under one scheduler write lock without changing per-match results. | **Code complete** — expiry/build/confirmation/missing groups update under one lock per group. |
| PF-17 | Perf-Nit | TEE oracle | Slice 1 | Config-time feed-id bytes/map are decoded once, not on every refresh; duplicate/missing feed behavior is unchanged. | **Code complete** — prepared lookup built once at task startup; invalid config pauses every bound market. |
...
- Live evidence: because the journal/settle pipeline changes, run the standard
  ...
  ALT, settle, and total timings. Until then PF-12 is `Code complete`.
...
**Still owed from slice 1:** hosted CI and review; then a digest-pinned CPU-CVM
real settle plus the standard recovery/drain drill. Record
`journal_write_us` count/p50/p95 and witness/prove, ALT, settle, and total
timings. Rows remain `Code complete` until that live evidence is attached.
