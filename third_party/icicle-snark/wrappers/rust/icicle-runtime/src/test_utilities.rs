...
pub static TEST_MAIN_DEVICE: Lazy<Mutex<Device>> =
    Lazy::new(|| Mutex::new(Device::new("UNKNOWN", 0)));
...
pub fn test_load_and_init_devices() {
    INIT.get_or_init(move || {
        ...
        let mut main_device = TEST_MAIN_DEVICE.lock().unwrap();
...
pub fn test_set_main_device() {
    let main_device = TEST_MAIN_DEVICE.lock().unwrap();
...
pub fn test_set_ref_device() {
    let ref_device = TEST_REF_DEVICE.lock().unwrap();
