...
extern "C" {
    ...
    pub fn icicle_copy_to_device(dst: *mut c_void, src: *const c_void, size: usize)
        -> eIcicleError;
...
pub fn get_registered_devices() -> Result<Vec<String>, eIcicleError> {
    ...
    unsafe {
        ...
        let devices: Vec<String> = str_slice.split(',').map(|s| s.to_string()).collect();
        Ok(devices)
    }
}
