...
use crate::{
    ntt::{
        get_root_of_unity, initialize_domain, ntt, ntt_inplace, release_domain, NTTConfig, NTTDir,
        NTTDomain, NTTInitDomainConfig, NTT,
    },
    ...
    <F as FieldImpl>::Config: NTT<F, F> + GenerateRandom<F>,
{
    ...
    for test_size in test_sizes {
        ...
        for alg in [NttAlgorithm::Radix2, NttAlgorithm::MixedRadix] {
            config.ext.set_int(CUDA_NTT_ALGORITHM, alg as i32);
    ...
    <F as FieldImpl>::Config: NTT<F, F> + GenerateRandom<F>,
{
    ...
    for test_size in test_sizes {
        ...
        for alg in [NttAlgorithm::Radix2, NttAlgorithm::MixedRadix] {
            ...
            .unwrap();
            assert_eq!(
                *ntt_result_half.as_slice(),
                ntt_large_result.as_slice()[..small_size]
            );
            assert_eq!(
                *ntt_result_coset.as_slice(),
                ntt_large_result.as_slice()[small_size..]
            );
        ...
        .for_each(move |device_id| {
            ...
            for test_size in test_sizes {
                ...
                for batch_size in batch_sizes {
                    ...
                    let mut scalars_d =
                        DeviceVec::<F>::device_malloc(test_size * batch_size).unwrap();
                    scalars_d
                    ...
                    for coset_gen in coset_generators {
                        for ordering in [Ordering::kNN, Ordering::kRR] {
                            ...
                            test_utilities::test_set_ref_device();
                            ntt_inplace(
                                HostSlice::from_mut_slice(&mut scalars_clone),
                                NTTDir::kForward,
                                &config,
                            )
                            .unwrap();
                            ...
                            for alg in [NttAlgorithm::Radix2, NttAlgorithm::MixedRadix] {
                                ...
                                let mut ntt_result_slice =
                                    HostSlice::from_mut_slice(&mut ntt_result_h);
