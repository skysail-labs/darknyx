...
pub fn generate_random_affine_points_with_zeroes<C: Curve>(
    size: usize,
    num_zeroes: usize,
) -> Vec<Affine<C>> {
    ...
    C::ScalarField: MontgomeryConvertible,
{
    ...
    (0..device_count) // TODO: this is proto-loadbalancer
        ...
        .for_each(move |device_id| {
            ...
            let mut msm_results =
                DeviceVec::<Projective<C>>::device_malloc_async(1, &stream).unwrap();
            for test_size in test_sizes {
                ...
                test_utilities::test_set_main_device_with_id(device_id);
                let mut scalars_d =
                    DeviceVec::<C::ScalarField>::device_malloc_async(test_size, &stream).unwrap();
                scalars_d
    ...
    <C::ScalarField as FieldImpl>::Config: GenerateRandom<C::ScalarField>,
{
    ...
    for test_size in test_sizes {
        ...
        let mut precomputed_points_d =
            DeviceVec::<Affine<C>>::device_malloc(cfg.precompute_factor as usize * test_size)
                .unwrap();
        precompute_bases(
            HostSlice::from_slice(&points),
            &cfg,
        ...
        for batch_size in batch_sizes {
            let scalars =
                <C::ScalarField as FieldImpl>::Config::generate_random(test_size * batch_size);
    ...
    <C::ScalarField as FieldImpl>::Config: GenerateRandom<C::ScalarField>,
{
    ...
    for test_size in test_sizes {
        ...
        for batch_size in batch_sizes {
            ...
            let mut precomputed_points_d = DeviceVec::<Affine<C>>::device_malloc(
                cfg.precompute_factor as usize * test_size * batch_size,
            )
            .unwrap();
            ...
            let mut msm_results_2 = DeviceVec::<Projective<C>>::device_malloc(batch_size).unwrap();
            let mut points_d =
                DeviceVec::<Affine<C>>::device_malloc(test_size * batch_size).unwrap();
            points_d
                .copy_from_host_async(HostSlice::from_slice(&points), &stream)
                .unwrap();
    ...
    <C::ScalarField as FieldImpl>::Config: GenerateRandom<C::ScalarField>,
{
    ...
    for test_size in test_sizes {
        for batch_size in batch_sizes {
            let points =
                generate_random_affine_points_with_zeroes::<C>(test_size * batch_size, 100);
