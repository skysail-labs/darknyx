            ...
            for test_size_log2 in (min_log2..=max_log2) {
                ...
                for precompute_factor in [1, 4, 8] {
                    let mut precomputed_points_d =
                        DeviceVec::device_malloc(precompute_factor * test_size).unwrap();
                    ...
                    for batch_size_log2 in [0, 4, 7] {
                        ...
                        let mut scalars =
                            <C::ScalarField as FieldImpl>::Config::generate_random(full_size);
                        let scalars =
                            <C::ScalarField as FieldImpl>::Config::generate_random(full_size);
                        ...
                        let mut msm_results =
                            DeviceVec::<Projective<C>>::device_malloc(batch_size).unwrap();
