...
7. [The ZK circuits](#7-the-zk-circuits)
...
shielded (UTXO notes, Groth16 proofs); hidden order intake, matching, and proving
run in an Intel TDX CVM that drives signed atomic settlement on L1. The CVM's K
Ed25519 shard signers are derived by dstack KMS and bound into the quote that
clients verify before disclosing order intent.
...
- **Settle-time proof.** `verify_match_batch` checks ONE `VALID_MATCH_BATCH`
  ...
  `close_batch_validity_marker` reclaims the marker's rent only at or after its
  on-chain-derived expiry.
...
| TEE repeatedly selects a colluding market maker | **TEE-trusted, externally monitored.** Attestation proves which matcher code is running but cannot make a compromised enclave apply its private counterparty-selection policy honestly. Launch therefore requires published per-market-maker selection share, price improvement, settlement-failure rate, and settlement latency so persistent preferential routing is externally detectable. |
...
| Trusted-setup ceremony soundness | **Open** — all five Groth16 circuit families (eight compiled instances) use a deterministic dev contribution. Real Phase-2 MPC required for mainnet. |
...
| Order intent in transit (T-03) | **Programmatic path: in-enclave RA-TLS, with lifecycle hardening still open.** `darknyx-tee` generates a boot-random, memory-only TLS key and serves TLS 1.3 on `:8443`; dstack's `-8443s` route passes that TLS stream through without terminating it. A separate nonce-bound transport quote commits to the served certificate SPKI, boot session, measured deployment and full ordered signer set. The CPU production compose does not publish plaintext `:8080`. The daemon/SDK verify the certificate on the connection they use and gate the WebSocket upgrade before sending frames. After bootstrap, the connector refuses a replacement socket whose SPKI differs from the quote-bound boot key before application bytes can leave (audit_9 TR-02, code complete pending merge). TR-05 still tracks supervised recovery after a genuine boot rotation. **Deferred surfaces:** the ordinary `trader-host` still sees browser order and stream plaintext, and GPU transport parity awaits confidential-GPU access. Neither is launch-qualified. See `docs/transport-integrity-remediation-plan.md` and `audits/residual-backlog.md`. Network timing and volume remain observable on every path. |
...
- **Bind the traders' signed limits in-circuit.** The `clearing_price ≤ limit`
  ...
  constraints; the N=16 proof would blow up from seconds to minutes–hours). The only
...
- **Bind the oracle band on-chain.** Matching happens at T0 (the matcher's TWAP);
  `verify_match_batch` runs at T1 = T0 + lock + prove + tx latency, when the
...
| **Groth16** (BN254, snarkjs / `groth16-solana`) | Five circuit families (eight compiled instances) | Constant-size proofs (256 bytes on-chain), constant-time verification, well-supported tooling. The proof system that fits Solana's CU budget. |
...
settlement: it remains a private proof witness or is carried only inside the
attested CVM. The public protocol fee owner commitment in `VaultConfig` is a
different, intentionally governed value.
...
No wallet-registration PDA or wallet-create circuit remains. The owner
commitment is validated privately inside the active note circuits; publishing
it beside a Solana signer would create a permanent identity edge without
granting any authority the protocol uses.
...
their unlinkable use tags key `ConsumedNoteEntry` PDAs): `note_a` (buyer's
...
On-chain, `tee_forced_settle_batched` creates a fresh `NoteLock` PDA
   (`create_relock_pda`) seeded by the change note's use tag, bound to the
...
a process-local counter. The canonical-order clean cutover removed anchor
fields and the top-up endpoint; the current signed order domain is v4.
...
corresponding `NoteLock` PDA to be absent or expired before proof verification
or state mutation, so a wallet cannot merge collateral reserved by a live
order. See §7.6.
...
Five Groth16 circuit families compile into eight instances: `VALID_DEPOSIT`,
`VALID_SPEND`, `VALID_INPUT`, `VALID_MERGE` at K=2/K=4, and
`VALID_MATCH_BATCH` at N=2/N=4/N=16. The matching/settlement validity proof,
...
`groth16-solana v0.2.0`, which uses Solana's `alt_bn128` syscalls. Verification
cost varies with the public-signal count: the compressed two-input
...
// give it one. See the warning below — this is not cosmetic.
...
> position at all — by replaying any withdraw that **lands and reverts**. A
> reverted transaction publishes its proof in the ledger permanently while
> creating neither guard PDA, leaving the note spendable.

> **Why the squaring is load-bearing.** A signal that appears in no constraint
> has a **zero QAP polynomial**, so its IC point never enters `vk_x` and *any*
> value satisfies the pairing. Declaring `recipient` public without constraining
> it would produce a verifier that looks correct, compiles, and binds nothing.
...
**Public inputs** (4):
1. `merkleRoot`
2. `noteUseTag` — the circuit-derived, unlinkable seed for the on-chain
   `NoteLock` and shared consume-once PDA
...
this declared mint, owned by me, whose private amount is a positive u64; and
this public tag is derived from that exact leaf and its private inner hash."

The CVM verifies this proof at intake and later *relays* it on-chain, where
`lock_note` verifies it again. It cannot forge a proof for another note because
it never receives the spending key. The proof authorises a note and mint, not a
particular order: while the root remains in the shard's recent-root ring, an
authorised malicious CVM could replay the proof under a different `order_id`
for a bounded lock. That accepted S-08 boundary is described in §2.
...
**Attack without ownership constraint**: Merkle inclusion proves only that a
commitment exists. If the circuit accepted an arbitrary opening without
checking the spending-key relation, anyone who obtained a note opening (for
example through endpoint compromise, client leakage, or a malicious relay)
could generate a fresh VALID_INPUT proof and lock the note. Deposit no longer
...
and a large speedup in TEE-side proof generation (one batched proof
...
```
            ...
            │    • input/relock note-use tags        │
            │    • leaf_i := Poseidon12(active,      │
            │      input tags, output/fee commits,   │
            │      batch_slot, relock digest)         │
            └────────────────┬───────────────────────┘
                             │
...
- `config_digest` — `Poseidon10(DOMAIN_MATCH_CONFIG_V2=37, fee_rate_bps,
  protocol_owner_commitment, base_mint_lo, base_mint_hi, quote_mint_lo,
  quote_mint_hi, price_scale, fee_key_binding, fee_key_epoch)`. The preimage
  fields remain private circuit inputs and drive every slot. The vault
  recomputes the digest from the authoritative `VaultConfig` + `MarketConfig`;
  it is never accepted from the prover or cached across those independently
  updated accounts.

Leaf-hash construction. **Amount privacy remains:** the leaf hashes no plaintext
amount/mint/price. Input commitments are replaced by circuit-derived note-use
tags so Tx D does not republish its consumed leaves; output and fee commitments
remain because the handler appends them. Change-note tags are folded into a
relock digest to fit the on-chain `light-poseidon` arity cap of 12:

```
relock_digest = Poseidon3(DOMAIN_RELOCK_DIGEST = 30,
                          note_e_use_tag, note_f_use_tag)
leaf = Poseidon12(DOMAIN_LEAF_V3 = 31, active,
                  note_a_use_tag, note_b_use_tag,
                  note_c_commit, note_d_commit,
                  note_e_commit, note_f_commit,
...
constraints. Note-use tag derivation raised the current N=16 circuit from
234,025 to 285,401 constraints, crossing the 2^18 ceiling; it therefore uses
`pot19` (N=4 moved to pot18). Do not downgrade the PTAU assignment without
checking the compiled constraint count.
The pre-change prod9 CPU baseline used native
Circom witness generation plus rapidsnark and is roughly 0.3 s witness + 2.2 s
prove-step in steady-state measurements. Host class, warmup, cgroup throttling,
and backend materially change these figures; use the committed benchmark
artifacts and the note-use-tag CVM evidence for comparisons. The on-chain
verifier is measured in CU, not seconds.
...
```
            ...
            │  inputUseTags[i] :=                         │
            │     isActive[i]·Poseidon3(29,               │
            │       commit_i, innerHash[i])               │  ← dummy ⇒ public 0
...
sets `isActive[i]=0`, has zero amount, emits `inputUseTags[i]===0`, and
...
use tag (the replay guard), so a padded slot can't smuggle a spend. Its
...
- `deposited_note` (PDA at `[b"deposited_note", note_commitment]`, **init** —
  the deposit-once guard)
...
2. `init` the `deposited_note` PDA. Existence means this exact commitment is
   already a leaf, so the second deposit fails here rather than silently
   stranding value — see the note below.
3. SPL `transfer_checked` 5,015 USDC from Alice → vault_token_account.
4. `append_leaf(note_commitment)` — incremental Merkle update.
5. `outstanding_mint.outstanding += 5_015` (with `u64::checked_add`).
6. Assert `outstanding_mint.outstanding ≤ vault_token_account.amount` (post-reload).

> **Why a deposit-once guard.** Without it, two deposits sharing a commitment
> both moved tokens in and both incremented `outstanding`, but only ONE could
> ever be withdrawn — identical commitments + inners derive the same note-use
> tag, so the second collides on the tag-keyed consume-once guard. The vault
...
> any concurrent deposit would invalidate the proof. This PDA is the version
> that does not trade liveness for it.
...
only this route. Replacement sockets are refused against the boot's quote-bound
SPKI before dispatch. The remaining restart-supervision work is tracked in
`docs/transport-integrity-remediation-plan.md`; the deferred browser path
...
3. Verifies the VALID_INPUT proof against the selected shard's recent-root
   mirror before reserving collateral; L1 verifies the same proof again if the
   note is later locked.
4. Rejects stale boot sessions, non-contributory X25519 keys, and—after exact
   idempotency—non-increasing nonces for the same trading key.
5. Derives the fee-inclusive collateral (`nominal + own fee`) and books the
   order in the matcher for the requested configured symbol.
...
tick, over that market's in-memory book:
...
`MatchResultPayload`s. A venue-wide semaphore bounds batch concurrency
(`DARKNYX_TEE_SETTLE_BATCH_CONCURRENCY`, default 1); within a batch the worker
sends independent lock/settle transactions concurrently where dependencies
...
4. Construct the VALID_INPUT public inputs:
   `[merkle_root, note_use_tag, mint_lo, mint_hi]`.
...
Each independent lock transaction carries its own compute-budget instruction
for one Groth16 verification plus the account and Merkle checks.
...
vault::verify_match_batch(
    merkle_root:             [u8; 32],     // Poseidon root over per-slot leaves
    proof:                   Groth16Proof, // VALID_MATCH_BATCH at N=16
    fee_key_epoch:           u64,          // must equal governed epoch
    fee_recovery_ciphertext: [u8; 272],    // authenticated off-chain recovery
)
```

Accounts:
- `payer` (signer — must be an authorized TEE key; authenticates the encrypted
  fee-recovery record)
- `vault_config` (**ro** — source of the authoritative `fee_rate_bps` +
  `protocol_owner_commitment` + `fee_key_binding` + `fee_key_epoch`
...
Handler:
1. Derive `expiry_slot = clock.slot + 300`. It is not caller-controlled; this
   prevents a third party from front-running a valid proof with a one-slot
   marker that strands the already-landed locks.
2. Read fee rate, protocol owner, fee-key binding, and fee-key epoch from
   `vault_config`; require the caller's epoch to match; require the supplied
   market enabled with a nonzero scale; recompute
   `Poseidon10(37, fee, owner, base_lo, base_hi, quote_lo, quote_hi, scale,
   fee_key_binding, fee_key_epoch)`; and
...
The seven plaintext amount fields were removed in v7 and the two vestigial
nullifiers in v9. Amounts remain private circuit witnesses; tag-keyed
...
> unconstrained (no nullifier signal in VALID_MATCH_BATCH), so writing them
> provided no soundness — and it enabled a
...
> which would then collide. The tag-keyed `consumed_a/b` are the real
> double-spend guard, and `withdraw` now writes a matching `ConsumedNoteEntry`
> so the guard is symmetric across both paths. Withdraw's nullifier-keyed entry
> was itself dropped afterwards, closing that griefing vector on both sides.
...
ix data = disc(8) ‖ tree_id(1) ‖ payload(552) ‖ match_index(1) ‖ 4×32 siblings = 690 B.
...
4. **Validity marker check**:
   - Recompute the per-slot Merkle leaf via the same arity-12 construction the
     circuit uses (see §7.5). It binds input tags, created commitments, and the
     digest of the two change-note relock tags without hashing plaintext
     amounts/mints/price:
     ```
     relock = Poseidon3(30, note_e_use_tag, note_f_use_tag)
     leaf = Poseidon12(DOMAIN_LEAF_V3 = 31, active = 1,
                       note_a_use_tag, note_b_use_tag,
                       note_c, note_d, note_e, note_f,
                       note_fee_base, note_fee_quote, batch_slot, relock)
     ```
...
7. **Consumed-note allocation**: `ConsumedNoteEntry` PDAs at
   `[b"consumed_note", note_a_use_tag]` and `[b"consumed_note", note_b_use_tag]`.
   ...
   tag-keyed PDA, so a settle cannot consume an already-withdrawn note (and
   vice-versa). The tag is public and circuit-bound to the commitment + private
   inner hash, unlike the TEE-supplied settle nullifier.
...
9. **Append output leaves to the Merkle tree**: in this order:
   ...
   - `note_fee_base` (only if its commitment is nonzero)
   - `note_fee_quote` (only if its commitment is nonzero)
...
- **SHA-256** canonical settlement-digest recomputation
- **Poseidon12 + Poseidon3 + Poseidon2** for the match leaf, relock-tag digest,
  and depth-4 batch inclusion
...
- `tests/settle-builder-batched.test.ts` — settle ix wire-format
  ...
  12-account ordering, 690-byte ix data (disc + tree_id + payload + match_index +
...
She generates a VALID_SPEND proof off-chain via the SDK/snarkjs adapter.
Browser latency is device- and backend-dependent and remains a release
measurement rather than a protocol constant.
...
Accounts (11):
- `payer` (mut, signer — anyone can pay)
- `vault_config` (read-only — the SPL transfer authority; no tree state here)
- `merkle_tree[tree_id]` (read-only — the shard whose root ring is checked)
- `token_mint`, `vault_token_account` (mut), `destination_token_account` (mut)
- `consumed_note` (init — the tag-keyed consume-once guard)
- `note_lock_slot` (CHECK: must not be **live**)
...
3. **Layer-1 guard**: reject only if the `NoteLock` is **live**
   (`note_lock_is_live`, i.e. `clock.slot < expiry_slot`). An EXPIRED lock no
   longer blocks. This used to reject any initialized lock account regardless of
   expiry while nothing shipped could call `release_lock`, so one failed settle
   made a note permanently unwithdrawable, unmergeable, and unreleasable through
   every available interface — `MAX_LOCK_TTL_SLOTS` was documented as a bounded
   censorship window but was in practice unbounded.
4. **Recency check**: `merkle_tree[tree_id].contains_root(&merkle_root)` — must
   be in that shard's **64**-root ring buffer.
...
- `programs/vault/tests/zk_spend_roundtrip.rs` — real proof/on-chain verifier
- `tests/devnet-deposit-withdraw.test.ts` — isolated real devnet round-trip
...
WITHDRAW (L1, VALID_SPEND proof)
    ...
    │  SPL transferred out
    │  ConsumedNoteEntry PDA allocated → permanent burn
    │    (tag-keyed; the SAME guard settle uses, so the two
    │     paths block each other in both directions)
```
...
non-blocking; background permissionless sweepers then close expired lock and
marker accounts and return rent to the recorded payer. Accounts do not erase
themselves merely because a slot threshold passed.
...
canonical_payload_hash(p) = SHA256(
    ...
    p.note_e_use_tag, p.note_f_use_tag,
...
**What it catches independently of the settlement proof**: a liability/accounting
drift for a mint (for example, an implementation bug in deposit or withdrawal).
`VALID_MATCH_BATCH` now proof-binds the market mints and every output commitment,
so the historical fake-output-mint attack is rejected at batch verification.

With the outstanding counter, the withdraw rejects at
`InsufficientOutstanding` *before* attempting the SPL transfer — clean
error, clear logs.

The counter remains useful as defence-in-depth and as a clean error surface for
off-by-one accounting bugs.
...
**Cross-layer**: `withdraw` and settle both initialize the same
`ConsumedNoteEntry`. `withdraw` additionally rejects a **live** NoteLock but
allows an expired one. This handles both directions:
- Once a note is consumed by `tee_forced_settle` (layer 2 created), the
  user can no longer withdraw it via `VALID_SPEND` (the same layer-2 init
  collides).
- While a note is locked for an active order (layer 1 created and unexpired), the user
  can't withdraw it out from under the lock.

The note can only "exit" once — via settle, withdraw, or merge — and all three
paths burn the same layer-2 tag-keyed guard.
...
prove → fixture dump). Optional ICICLE CPU/CUDA parity tests are separately
feature- and hardware-gated.
...
| `cvm-multi-market-settle.test.ts` | `RUN_CVM_E2E=1` | two configured mint pairs settle independently through one CVM |
| `cvm-self-trade.test.ts` / `cvm-merge-then-order.test.ts` | `RUN_CVM_E2E=1` | owner-level self-trade rejection and merge-output spendability |
...
1. **Real Phase-2 ceremony** — every shipped Groth16 circuit uses a
   ...
   mainnet blocker. Need a public Phase-2 ceremony with at least five
   independent contributors, transcript/artifact hashes, a final random beacon,
   reproducible `snarkjs zkey verify`, and auditor sign-off. The PTAU files are SHA-256-pinned in
...
2. **On-chain attestation enforcement** — `set_tee_pubkey` rotates
   `VaultConfig.tee_pubkeys` (the K shard signer keys) to the CVM's
   dstack-derived keys. Clients already verify DCAP, replay RTMR3, pin the
   measurement, and compare the full quoted signer set with finalized chain
   state before sending orders. Governance independently verifies every
   attestation before rotation. The deferred stronger model is verifying that
   evidence on-chain rather than trusting the root/upgrade multisig. See
   `docs/tee-attestation-flow.md`.
...
Run one leaf-count CVM test per reset + cold boot. Stop a billable **CPU** CVM
after the test window. Never stop a prepaid on-demand GPU CVM: stopping
deallocates the instance permanently. The exact lifecycle is in
`docs/cvm-run-runbook.md` and `docs/gpu-tee-runbook.md`.

CI configuration and local commands evolve; `CLAUDE.md §2.5` is the
authoritative pre-PR gate. Environment-gated devnet/CVM validation remains
manual when hosted artifact capacity is unavailable.

---

*Last updated: 2026-08-28 — current architecture: `vault` (the only on-chain
program) + the multi-market in-CVM matcher/settler (`crates/darknyx-tee`),
validated end-to-end on devnet through Phala CPU and H200 CVMs. The as-built
privacy model uses commitment-once note leaves, unlinkable tag-keyed use,
single-secret ownership, private-inner-derived merge/continuation outputs,
epoch-keyed recoverable fee notes, compact exact replay markers,
recipient-bound withdrawals, proof-verified intake, and canonical order v5.
The unused wallet-registration circuit and the standalone `VALID_CREATE` /
`VALID_PRICE` circuits have been removed.*
