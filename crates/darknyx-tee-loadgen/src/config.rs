...
#[command(
    ...
    about = "Synthetic order traffic generator for darknyx-tee. \
             See docs/tee-architecture.md §13.4."
)]
pub struct RunConfig {
    /// Base URL of the running `darknyx-tee` instance. Examples:
    /// `http://127.0.0.1:8080` (local simulator) or
    /// `https://darknyx-tee-spike.example.com` (Phala devnet CVM).
    #[arg(long)]
    pub endpoint: String,
    ...
    /// `DARKNYX_TEE_API_KEY` first; the public fixture default is only suitable for
    /// an explicitly configured local simulator.
    #[arg(
        long,
        env = "DARKNYX_TEE_API_KEY",
        default_value = "darknyx-test-api-key"
    )]
    pub api_key: String,

    /// Override `api_secret`. Prefer the `DARKNYX_TEE_API_SECRET` environment
    /// variable for a Phala run so the secret is not exposed in process args.
    #[arg(
        long,
        env = "DARKNYX_TEE_API_SECRET",
        default_value = "darknyx-test-secret"
    )]
    pub api_secret: String,

    /// Override `passphrase`; reads `DARKNYX_TEE_PASSPHRASE` first.
    #[arg(
        long,
        env = "DARKNYX_TEE_PASSPHRASE",
        default_value = "darknyx-test-passphrase"
    )]
    pub passphrase: String,

    /// Protocol fee rate (bps) the target CVM is running
    /// (`DARKNYX_TEE_FEE_RATE_BPS`). The loadgen folds this into each
