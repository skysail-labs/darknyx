//! `darknyx-tee-loadgen` binary entry point.
//!
//! ```text
//! cargo run -p darknyx-tee-loadgen -- \
...
use anyhow::Result;
use clap::Parser;
use darknyx_tee_loadgen::{run_load_gen, RunConfig};
...
async fn main() -> Result<()> {
    ...
    if cfg.real_settle {
        ...
        {
            use darknyx_tee_loadgen::real_settle::run::{
...
fn init_tracing() {
    let filter = EnvFilter::try_from_env("DARKNYX_LOADGEN_LOG")
        .unwrap_or_else(|_| EnvFilter::new("info,darknyx_tee_loadgen=debug"));
