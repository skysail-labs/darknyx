...
//!
//! Spins up a real `darknyx-tee` instance in-process on an ephemeral
...
async fn loadgen_drives_real_tee_and_produces_matches() {
    ...
    let cfg = RunConfig {
        ...
        warmup_batches: 1,
        benchmark_label: "smoke".to_string(),
        metrics_json: None,
        cancel_rate: 0.20,
        ...
        oracle_exponent: -8,
        report: None,
        api_key: "darknyx-test-api-key".to_string(),
        api_secret: "darknyx-test-secret".to_string(),
        passphrase: "darknyx-test-passphrase".to_string(),
    ...
    assert!(outcome
        .markdown_report
        .contains("# darknyx-tee-loadgen benchmark"));
