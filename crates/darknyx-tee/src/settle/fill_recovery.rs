...
pub const RECOVERY_V3_TRAILER: [u8; 8] = *b"DNYXREC3";
...
impl FillCiphertext {
    ...
    pub fn to_payload_bytes(&self) -> [u8; RECOVERY_LEN] {
        ...
        if !self.is_empty() {
            out[RECOVERY_LEN - RECOVERY_V3_TRAILER.len()..].copy_from_slice(&RECOVERY_V3_TRAILER);
        }
        out
    }
    ...
    /// the empty sentinel; callers must never interpret legacy v1/v2 bytes
    /// using the v3 namespace and plaintext schema.
    pub fn from_payload_bytes(b: &[u8; RECOVERY_LEN]) -> Self {
        if b == &[0u8; RECOVERY_LEN]
            || b[RECOVERY_LEN - RECOVERY_V3_TRAILER.len()..] != RECOVERY_V3_TRAILER
        {
            return Self::default();
        }
...
mod tests {
    ...
    fn payload_bytes_round_trip() {
        ...
        assert_eq!(&bytes[120..], &RECOVERY_V3_TRAILER);
