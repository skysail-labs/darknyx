...
//!
//! Run with: `cargo test -p darknyx-tee --test prover_roundtrip`
...
use darknyx_tee::prover::{dummy_slot, ArkMatchBatchProver, Prover};

/// Resolve the repo's `circuits/build` directory from the crate
/// manifest dir (`crates/darknyx-tee`). In the production image the
