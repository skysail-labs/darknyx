//! Output types of `run_batch`.
//!
//! Cross-language byte equality is pinned by:
//!   * `tests/parity.rs` — matcher behaviour and commitment construction.
//!   * `packages/sdk/tests/settle-builder-batched.test.ts` — TS settlement
...
/// `[0u8;16]` — TEE order intake rejects zero-id submissions at
...
pub struct RunBatchOutput {
    ...
    /// `circuit_breaker_bps` of `pyth_twap`.
