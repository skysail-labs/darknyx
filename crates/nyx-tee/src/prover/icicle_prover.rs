...
//! prove via `NYX_TEE_ICICLE_DEVICE` (default `CPU`).
//!
//! ## Threading
//!
...
pub struct IcicleMatchBatchProver {
    ...
    /// Selected by `NYX_TEE_WITNESS` exactly like the rapidsnark backend.
...
impl IcicleMatchBatchProver {
    ...
    pub fn load(circuits_build_dir: impl AsRef<Path>, n: usize) -> Result<Self, ProverError> {
        ...
        let device = std::env::var("NYX_TEE_ICICLE_DEVICE")
            .ok()
            .filter(|s| !s.is_empty())
